...
// node_modules/.pnpm/reveal.js@5.0.4/node_modules/reveal.js/plugin/markdown/markdown.js
var require_markdown = __commonJS({
  "node_modules/.pnpm/reveal.js@5.0.4/node_modules/reveal.js/plugin/markdown/markdown.js"(exports, module) {
    !function(e, t) {
      ...
      function f(e2, t2, n2) {
        ...
        t2 && !d.test(n2) && (n2 = function(e3, t3) {
          k[" " + e3] || (x.test(e3) ? k[" " + e3] = e3 + "/" : k[" " + e3] = _(e3, "/", true));
          e3 = k[" " + e3];
      ...
      class S {
        ...
        code(e2) {
          ...
          if (t2) {
            ...
            return { type: "code", raw: t2[0], codeBlockStyle: "indented", text: this.options.pedantic ? e3 : _(e3, "\n") };
          }
        }
        fences(e2) {
      ...
      const R = { escape: /^\\([!"#$%&'()*+,\-./:;<=>?@\[\]\\^_`{|}~])/, autolink: /^<(scheme:[^\s\x00-\x1f<>]*|email)>/, url: w, tag: "^comment|^</[a-zA-Z][\\w:-]*\\s*>|^<[a-zA-Z][\\w-]*(?:attribute)*?\\s*/?>|^<\\?[\\s\\S]*?\\?>|^<![a-zA-Z]+\\s[\\s\\S]*?>|^<!\\[CDATA\\[[\\s\\S]*?\\]\\]>", link: /^!?\[(label)\]\(\s*(href)(?:\s+(title))?\s*\)/, reflink: /^!?\[(label)\]\[(ref)\]/, nolink: /^!?\[(ref)\](?:\[\])?/, reflinkSearch: "reflink|nolink(?!\\()", emStrong: { lDelim: /^(?:\*+(?:([punct_])|[^\s*]))|^_+(?:([punct*])|([^\s_]))/, rDelimAst: /^(?:[^_*\\]|\\.)*?\_\_(?:[^_*\\]|\\.)*?\*(?:[^_*\\]|\\.)*?(?=\_\_)|(?:[^*\\]|\\.)+(?=[^*])|[punct_](\*+)(?=[\s]|$)|(?:[^punct*_\s\\]|\\.)(\*+)(?=[punct_\s]|$)|[punct_\s](\*+)(?=[^punct*_\s])|[\s](\*+)(?=[punct_])|[punct_](\*+)(?=[punct_])|(?:[^punct*_\s\\]|\\.)(\*+)(?=[^punct*_\s])/, rDelimUnd: /^(?:[^_*\\]|\\.)*?\*\*(?:[^_*\\]|\\.)*?\_(?:[^_*\\]|\\.)*?(?=\*\*)|(?:[^_\\]|\\.)+(?=[^_])|[punct*](\_+)(?=[\s]|$)|(?:[^punct*_\s\\]|\\.)(\_+)(?=[punct*\s]|$)|[punct*\s](\_+)(?=[^punct*_\s])|[\s](\_+)(?=[punct*])|[punct*](\_+)(?=[punct*])/ }, code: /^(`+)([^`]|[^`][\s\S]*?[^`])\1(?!`)/, br: /^( {2,}|\\)\n(?!\s*$)/, del: w, text: /^(`+|[^`])(?:(?= {2,}\n)|[\s\S]*?(?:(?=[\\<!\[`*_]|\b_|$)|[^ ](?= {2,}\n)))/, punctuation: /^([\spunctuation])/ };
      function A(e2) {
      ...
      class v {
        constructor(e2) {
          ...
          const n2 = { block: T.normal, inline: R.normal };
          this.options.pedantic ? (n2.block = T.pedantic, n2.inline = R.pedantic) : this.options.gfm && (n2.block = T.gfm, this.options.breaks ? n2.inline = R.breaks : n2.inline = R.gfm), this.tokenizer.rules = n2;
        }
        static get rules() {
          return { block: T, inline: R };
        }
        static lex(e2, t2) {
...
`;
      ...
      class C {
        ...
        parse(e2, t2 = true) {
          let n2, s2, r2, i2, l2, o2, a2, c2, p2, u2, g2, d2, f2, k2, x2, m2, b2, w2, y2, _2 = "";
          const $2 = e2.length;
          for (n2 = 0; n2 < $2; n2++)
            if (u2 = e2[n2], this.options.extensions && this.options.extensions.renderers && this.options.extensions.renderers[u2.type] && (y2 = this.options.extensions.renderers[u2.type].call({ parser: this }, u2), false !== y2 || !["space", "hr", "heading", "code", "table", "blockquote", "list", "html", "paragraph", "text"].includes(u2.type)))
              _2 += y2 || "";
            else
              switch (u2.type) {
                case "space":
                ...
                case "hr":
                  ...
                  continue;
                case "heading":
                  _2 += this.renderer.heading(this.parseInline(u2.tokens), u2.depth, h(this.parseInline(u2.tokens, this.textRenderer)), this.slugger);
                  continue;
                case "code":
                  _2 += this.renderer.code(u2.text, u2.lang, u2.escaped);
                  continue;
                case "table":
                  ...
                  _2 += this.renderer.table(c2, p2);
                  continue;
                case "blockquote":
                  p2 = this.parse(u2.tokens), _2 += this.renderer.blockquote(p2);
                  continue;
                case "list":
                  ...
                  _2 += this.renderer.list(p2, g2, d2);
                  continue;
                case "html":
                  _2 += this.renderer.html(u2.text);
                  continue;
                case "paragraph":
                  _2 += this.renderer.paragraph(this.parseInline(u2.tokens));
                  continue;
                case "text":
                  for (p2 = u2.tokens ? this.parseInline(u2.tokens) : u2.text; n2 + 1 < $2 && "text" === e2[n2 + 1].type; )
                    u2 = e2[++n2], p2 += "\n" + (u2.tokens ? this.parseInline(u2.tokens) : u2.text);
                  _2 += t2 ? this.renderer.paragraph(p2) : p2;
                  continue;
                default: {
      ...
      function M(e2, t2) {
        return (n2, s2, r2) => {
          ...
          const i2 = { ...s2 }, l2 = function(e3, t3, n3) {
            return (s3) => {
          ...
          }((s2 = { ...P.defaults, ...i2 }).silent, s2.async, r2);
          if (null == n2)
          ...
          if (function(e3) {
            ...
            const a2 = function(e3) {
              ...
              if (!e3)
                try {
                  s2.walkTokens && P.walkTokens(o2, s2.walkTokens), n3 = t2(o2, s2), s2.hooks && (n3 = s2.hooks.postprocess(n3));
                } catch (t3) {
                  e3 = t3;
                }
            ...
            if (delete s2.highlight, !o2.length)
              return a2();
            let c2 = 0;
            return P.walkTokens(o2, function(e3) {
          ...
          if (s2.async)
            return Promise.resolve(s2.hooks ? s2.hooks.preprocess(n2) : n2).then((t3) => e2(t3, s2)).then((e3) => s2.walkTokens ? Promise.all(P.walkTokens(e3, s2.walkTokens)).then(() => e3) : e3).then((e3) => t2(e3, s2)).then((e3) => s2.hooks ? s2.hooks.postprocess(e3) : e3).catch(l2);
          try {
            s2.hooks && (n2 = s2.hooks.preprocess(n2));
            const r3 = e2(n2, s2);
            s2.walkTokens && P.walkTokens(r3, s2.walkTokens);
            let i3 = t2(r3, s2);
      ...
      function P(e2, t2, n2) {
        return M(v.lex, C.parse)(e2, t2, n2);
      }
      P.options = P.setOptions = function(e2) {
        var n2;
        return P.defaults = { ...P.defaults, ...e2 }, n2 = P.defaults, t = n2, P;
      }, P.getDefaults = e, P.defaults = t, P.use = function(...e2) {
        const t2 = P.defaults.extensions || { renderers: {}, childTokens: {} };
        e2.forEach((e3) => {
          const n2 = { ...e3 };
          if (n2.async = P.defaults.async || n2.async || false, e3.extensions && (e3.extensions.forEach((e4) => {
            if (!e4.name)
            ...
            const t3 = P.defaults.renderer || new I();
          ...
          if (e3.tokenizer) {
            const t3 = P.defaults.tokenizer || new S();
          ...
          if (e3.hooks) {
            const t3 = P.defaults.hooks || new Z();
          ...
          if (e3.walkTokens) {
            const t3 = P.defaults.walkTokens;
      ...
      }, P.walkTokens = function(e2, t2) {
        let n2 = [];
        for (const s2 of e2)
          switch (n2 = n2.concat(t2.call(P, s2)), s2.type) {
            case "table":
              for (const e3 of s2.header)
                n2 = n2.concat(P.walkTokens(e3.tokens, t2));
              for (const e3 of s2.rows)
                for (const s3 of e3)
                  n2 = n2.concat(P.walkTokens(s3.tokens, t2));
              break;
            case "list":
              n2 = n2.concat(P.walkTokens(s2.items, t2));
              break;
            default:
              P.defaults.extensions && P.defaults.extensions.childTokens && P.defaults.extensions.childTokens[s2.type] ? P.defaults.extensions.childTokens[s2.type].forEach(function(e3) {
                n2 = n2.concat(P.walkTokens(s2[e3], t2));
              }) : s2.tokens && (n2 = n2.concat(P.walkTokens(s2.tokens, t2)));
          }
        return n2;
      }, P.parseInline = M(v.lexInline, C.parseInline), P.Parser = C, P.parser = C.parse, P.Renderer = I, P.TextRenderer = q, P.Lexer = v, P.lexer = v.lex, P.Tokenizer = S, P.Slugger = L, P.Hooks = Z, P.parse = P, P.options, P.setOptions, P.use, P.walkTokens, P.parseInline, C.parse, v.lex;
      const N = "__SCRIPT_END__", O = /\[\s*((\d*):)?\s*([\s\d,|-]*)\]/, D = { "&": "&amp;", "<": "&lt;", ">": "&gt;", '"': "&quot;", "'": "&#39;" };
      return () => {
        let e2;
        function t2(e3) {
        ...
        function r2(e3, t3) {
          ...
          return 2 === n3.length && (e3 = n3[0] + '<aside class="notes">' + P(n3[1].trim()) + "</aside>"), '<script type="text/template">' + (e3 = e3.replace(/<\/script>/g, N)) + "<\/script>";
        }
        function i2(e3, t3) {
          t3 = s2(t3);
        ...
        function c2() {
          ...
          return [].slice.call(n3).forEach(function(e3) {
            ...
            e3.innerHTML = P(s3), a2(e3, e3, null, e3.getAttribute("data-element-attributes") || e3.parentNode.getAttribute("data-element-attributes") || "\\.element\\s*?(.+?)$", e3.getAttribute("data-attributes") || e3.parentNode.getAttribute("data-attributes") || "\\.slide:\\s*?(\\S.+?)$"), n4 && e3.appendChild(n4);
          }), Promise.resolve();
        }
        return { id: "markdown", init: function(t3) {
          e2 = t3;
          let { renderer: n3, animateLists: s3, ...r3 } = e2.getConfig().markdown || {};
          return n3 || (n3 = new P.Renderer(), n3.code = (e3, t4) => {
            let n4 = "", s4 = "";
            if (O.test(t4)) {
              let e4 = t4.match(O)[2];
          ...
          }), true === s3 && (n3.listitem = (e3) => `<li class="fragment">${e3}</li>`), P.setOptions({ renderer: n3, ...r3 }), l2(e2.getRevealElement()).then(c2);
        }, processSlides: l2, convertSlides: c2, slidify: i2, marked: P };
      };
    });
  }
});
...
reveal.js/plugin/markdown/markdown.js:
  (*!
     ...
     *)
*/
//# sourceMappingURL=reveal__js_plugin_markdown_markdown.js.map
